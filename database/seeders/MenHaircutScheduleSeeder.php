...
class MenHaircutScheduleSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $scheduling = Scheduling::create([
            'name' => 'Mens haircut',
            'description' => 'Scheduling date for Mens haircut',
            'max_clients_per_slot' => 3,
        ]);

        //creating Men Haircut Service
        $menHaircutService = $scheduling->services()->create([
            'name' => 'Men Haircut',
            'duration' => 10,
        ]);
            ...
            if ($dayOfWeek !== CarbonInterface::SUNDAY) {
                ...
                while ($startTime < $endTime) {
                    ...
                    if (!($slotStartTime->between($cleaningBreakStart, $cleaningBreakEnd) || $slotStartTime->between($lunchBreakStart, $lunchBreakEnd))) {
                        ...
                        $slotEndTime = $slotEndTime->addMinutes($menHaircutService->duration);
                    ...
                    $startTime = $startTime->addMinutes($menHaircutService->duration + $cleanupBreak);
                }
            } else {
                //Sunday is closed
                $scheduling->holidays()
